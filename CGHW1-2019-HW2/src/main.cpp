...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        prog["Blinn_Phong"] = static_cast<int>(Blinn_Phong);
        if(vertex_lighting && isVertex){
            isVertex = false;
        ...
        }else if(!vertex_lighting && !isVertex){
            isVertex = true;
            prog = Program::LoadFromFile(
                "../resource/vs.vert",
                "../resource/gs.geom",
        ...
        if(flat_shading && isFLat){
            isFLat = false;
            mesh = StaticMesh::LoadMeshFlat("../resource/sphere.obj");
            std::cout << "Flat"<<std::endl;
        }else if(!flat_shading && !isFLat){
            isFLat = true;
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
        {
            ...
            ImGui::Checkbox("Vertex Lighting", &vertex_lighting);
